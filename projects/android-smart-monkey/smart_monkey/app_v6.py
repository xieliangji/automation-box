...
class SmartMonkeyAppV6(WatchdogSmartMonkeyApp):
    ...
    def run(self) -> None:
        ...
        for step in range(self.config.run.max_steps):
            ...
            if transition.out_of_app or self.should_escape(previous_state, current_state, next_state, transition):
                self._recover_with_navigation(current_state.state_id, transition.out_of_app)
    ...
    def _recover_with_navigation(self, current_state_id: str, out_of_app: bool) -> None:
        decision = self.backtrack_helper.choose(
            current_state_id=current_state_id,
            stuck_score=self.runtime.stats.stuck_score,
            out_of_app=out_of_app,
        )
        plan = self.navigation_recovery.build_plan(decision, max_actions=3)
        self.recorder.record_step(
            {
                "step": -1,
