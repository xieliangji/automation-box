...
class SmartMonkeyAppOrchestrated(WatchdogSmartMonkeyApp):
        ...
        for step in range(self.config.run.max_steps):
            ...
            if self.v2_config.features.enable_watchdog:
                ...
                if events:
                    ...
                    self.runtime_hooks.on_watchdog_events(issue_dirs)

            if transition.out_of_app or self.should_escape(previous_state, current_state, next_state, transition):
                self._recover_orchestrated(current_state.state_id, transition.out_of_app)

            previous_state = next_state

        self.runtime_hooks.on_run_finish(self.recorder, self.runtime.utg)

    def _recover_orchestrated(self, current_state_id: str, out_of_app: bool) -> None:
