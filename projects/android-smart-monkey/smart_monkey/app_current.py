...
class SmartMonkeyAppCurrent(WatchdogSmartMonkeyApp):
    ...
    def run(self) -> None:
        ...
        for step in range(self.config.run.max_steps):
            ...
            if transition.out_of_app or self.should_escape(previous_state, current_state, next_state, transition):
                self._recover_current(current_state.state_id, transition.out_of_app)

            previous_state = next_state

        self.recorder.record_utg(self.runtime.utg)
        self.report_service.generate_all()
    ...
    def persist_step(self, step, current_state, action, transition, next_state) -> None:
        ...
        self.telemetry_service.maybe_capture_periodic_snapshot(step=step, driver=self.driver, recorder=self.recorder)

    def _recover_current(self, current_state_id: str, out_of_app: bool) -> None:
