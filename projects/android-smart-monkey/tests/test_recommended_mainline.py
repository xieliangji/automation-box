...
class FakeDriver:
    ...
    def get_foreground_package(self) -> str:
        return "com.demo.app" if self.started else ""
...
class TestableRecommendedApp(SmartMonkeyAppRecommendedV2):
...
def test_recommended_mainline_generates_artifacts(tmp_path: Path) -> None:
    ...
    app.extractor.extract = lambda state: [
        Action(
            action_id="a1",
    ...
    app.scorer.score = lambda state, actions, stats: actions
